...
<div>
  ...
  <div class="mt-6 border-t border-gray-200">
    ...
    <h2 class="mt-4 font-semibold">Event Settings</h2>
          ...
          <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
            ...
            <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
              <input
                ...
                on:blur={(ev) => {
                  toast.promise(
                    data.pb.breakfast.events.setStoredDuration(ev.currentTarget.value),
                    {
            ...
            <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
              <Select.Root
                ...
                onOpenChange={(open) => {
                  if (open) return;
                  toast.promise(
                    data.pb.breakfast.events.setSavedTypes(selected.map((s) => s.value)),
                    {
                          ...
                          on:click={() => {
                            toast.promise(
                                ...
                                .then(() => invalidate("db:events")),
                              {
                                ...
                                error: (err) =>
                                  `Failed to delete event subscription: ${err.message}`,
                              },
                            );
                          }}><Trash size="0.75rem" /></button
                        >
                      </li>
                    {/each}
                  </ul>
