...
export class OBSWebSocket extends EventTarget {
  ...
  async connect(port: number, password: string): Promise<Result<void>> {
    if (this.#connected) throw new Error("Already connected");

    this.#ws = new WebSocket(`ws://localhost:${port}`, ["obswebsocket.msgpack"]);
    ...
    setTimeout(() => {
      ...
      rejector(new Error("Connection timed out"));
    }, 3000);
